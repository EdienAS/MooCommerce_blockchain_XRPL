                ...
                <div>
                    ...
                    <div class="tab-content customer-order-list">
                        <div role="tabpanel" class="tab-pane @if(request()->input('tab') == 'profile')active @endif" id="profile">
                            ...
                            @if($customer->blockchain_signed)
                                <small><b><a href="https://testnet.xrpl.org/nft/{{$customer->blockchain_nftid}}" target="blank">Blockchain Signed</a></b></small>
                            @endif
                        </div>
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <table class="table">
                                                              ...
                                                              @foreach ($order['products'] as $product)
                                                                @php
                                                                    $orderProduct = \App\OrderProducts::where('order_id', $order['id'])
                                                                                                ->where('product_sku', $product['sku'])
                                                                                                ->where('blockchain_nftid', '!=', "")
                                                                                                ->orderBy('id')
                                                                                                ->first();
                                                                    if($orderProduct){
                                                                        $qrcode = base64_encode(QrCode::format('svg')->size(200)->errorCorrection('H')->generate("http://".request()->getHttpHost()."/warrenty/card/".$orderProduct['blockchain_UUID']));
                                                                    }
